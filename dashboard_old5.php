...
for ($m = 1; $m <= 12; $m++) {
  ...
  $s  = $d->prepare("SELECT COALESCE(SUM(amount),0) FROM expenses WHERE DATE_FORMAT(expense_date,'%Y-%m')=? AND payment_status='unpaid'");
  $s->execute([$ym]); $year_unpaid[] = (float)$s->fetchColumn();
}
      ...
      <div class="card-panel-body border-top pt-3">
        <div class="row g-3 text-center">
          <div class="col-6 col-md-3">
            ...
            <div class="small text-muted">Total Paid <?= $report_year ?></div>
          </div>
          <div class="col-6 col-md-3">
            <div class="fw-semibold text-danger fs-6"><?= money(array_sum($year_unpaid)) ?></div>
            <div class="small text-muted">Total Unpaid <?= $report_year ?></div>
          </div>
          <div class="col-6 col-md-3">
            <div class="fw-semibold text-dark fs-6"><?= money(array_sum($year_paid) + array_sum($year_unpaid)) ?></div>
            <div class="small text-muted">Jumlah Keseluruhan</div>
          </div>
          <div class="col-6 col-md-3">
            <div class="fw-semibold text-primary fs-6">
              <?php $tot = array_sum($year_paid)+array_sum($year_unpaid);
